...
class SBCFormerBlock(nn.Module):  # building block
    def __init__(self, depth_invres, depth_mattn, depth_mixer, dim, key_dim, num_heads, mlp_ratio=4., attn_ratio=2,
                 drop=0., attn_drop=0.,
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, in_features, out_features):
        ...
        self.conv = nn.Conv2d(in_channels=in_features, out_channels=out_features, kernel_size=3, stride=2, padding=1,
                              bias=False)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, embed_dim=64):
        ...
        self.stem = nn.Sequential(
            ...
            Conv2d_BN(in_features=self.embed_dim // 4, out_features=self.embed_dim // 2, kernel_size=3, stride=2,
                      padding=1),
            nn.ReLU(inplace=True),
...
class SBCFormer(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[128, 320, 512], key_dim=32,
                 num_heads=[2, 4, 8], attn_ratio=2, mlp_ratio=4, invres_ratio=2,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., depths_invres=[2, 2, 1], depths_mattn=[1, 4, 3],
                 depths_mixer=[2, 2, 2], pool_ratios=[4, 2, 1]):
        super().__init__()
        ...
        for i in range(self.num_stages - 1):
            self.merging_blocks.append(
                PatchMerging(in_features=self.embed_dims[i], out_features=self.embed_dims[i + 1]))

        self.dpr = [x.item() for x in
                    torch.linspace(0, drop_path_rate, sum(self.depths_mattn))]  # stochastic depth decay rule
        cur = 0
        for i in range(self.num_stages):
            self.sbcformer_blk = SBCFormerBlock(depth_invres=self.depths_invres[i], depth_mattn=self.depths_mattn[i],
                                                depth_mixer=self.depths_mixer[i], dim=self.embed_dims[i],
                                                key_dim=key_dim, num_heads=self.num_heads[i],
                                                mlp_ratio=mlp_ratio, attn_ratio=attn_ratio, drop=drop_rate,
                                                attn_drop=attn_drop_rate,
                                                drop_paths=self.dpr[cur: cur + self.depths_mattn[i]],
                                                pool_ratio=pool_ratios[i], invres_ratio=invres_ratio)
...
def SBCFormer_XS(pretrained=False, **kwargs):
    model = SBCFormer(
        img_size=224, embed_dims=[96, 160, 288], key_dim=16, num_heads=[3, 5, 6], invres_ratio=2, attn_ratio=2,
        pool_ratios=[4, 2, 1],
...
def SBCFormer_S(pretrained=False, **kwargs):
    model = SBCFormer(
        img_size=224, embed_dims=[96, 192, 320], key_dim=16, num_heads=[3, 5, 7], invres_ratio=2, attn_ratio=2,
        pool_ratios=[4, 2, 1],
...
def SBCFormer_B(pretrained=False, **kwargs):
    model = SBCFormer(
        img_size=224, embed_dims=[128, 256, 384], key_dim=24, num_heads=[4, 6, 8], invres_ratio=2, attn_ratio=2,
        pool_ratios=[4, 2, 1],
        depths_invres=[2, 2, 1], depths_mattn=[2, 4, 3], depths_mixer=[2, 2, 2], **kwargs)
    model.default_cfg = _cfg()
    return model
...
def SBCFormer_L(pretrained=False, **kwargs):
    model = SBCFormer(
        img_size=224, embed_dims=[192, 288, 384], key_dim=32, num_heads=[4, 6, 8], invres_ratio=4, attn_ratio=2,
        pool_ratios=[4, 2, 1],
        depths_invres=[2, 2, 1], depths_mattn=[2, 4, 3], depths_mixer=[2, 2, 2], **kwargs)
    model.default_cfg = _cfg()
    return model