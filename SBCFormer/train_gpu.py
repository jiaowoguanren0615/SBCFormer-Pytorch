...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data-root', default='/flower_data', help='dataset path')
    parser.add_argument('--nb_classes', type=int, default=5, help='number classes of dataset')
    ...
    parser.add_argument('--pin-mem', type=bool, default=True,
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if test_stats["acc1"] > max_accuracy:
            if args.output_dir:
                checkpoint_paths = [
                    output_dir / '{}_checkpoint.pth'.format(args.model)]
        ...
        if args.output_dir and utils.is_main_process():
            with (output_dir / "{}_log.txt".format(args.model)).open("a") as f:
    ...
    Predictor(model_without_ddp, data_loader_val, f'{args.output_dir}/{args.model}_checkpoint.pth', args.device)
    Plot_ROC(model_without_ddp, data_loader_val, f'{args.output_dir}/{args.model}_checkpoint.pth', args.device)


if __name__ == '__main__':
    parser = argparse.ArgumentParser('sbcformer training and evaluation script', parents=[get_args_parser()])
